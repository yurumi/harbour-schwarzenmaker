...
QVariant FileModel::data(const QModelIndex &index, int role) const
{
    ...
    case FileIconRole:
      return "icon";//infoToIconName(info);

    case PermissionsRole:
      return "xxx";//permissionsToString(info.permissions());

    case SizeRole:
        if (info.isSymLink() && info.isDirAtEnd()) return tr("dir-link");
        if (info.isDir()) return tr("dir");
        return "33";//filesizeToString(info.size());

    case LastModifiedRole:
      return "iae";//datetimeToString(info.lastModified());

    case CreatedRole:
      return "ia";//datetimeToString(info.created());
...
void FileModel::readAllEntries()
{
    ...
    if (access(m_dir.toStdString().c_str(), R_OK) == -1) {
